  ...
  <header>
    <div class="container">
      ...
      <nav>
        ...
        <a href="sign-up.html"><i class="far fa-envelope fa-lg"></i></a>
...
$jobsData = getJobsDataByCategoryAndLimit($conn, $category, $limit);
...
foreach ($categories as $category) {
    ...
    echo '         <div class="line">';
    echo '            <a href="remote-jobs.php" ">See All Jobs ></a>';
    echo '        </div>';    
    echo '    </div>';
    echo '</div>';
}
...
?>
  ...
  document.addEventListener("DOMContentLoaded", function() {
    const seeJobsButtons = document.querySelectorAll(".see-jobs-button");
    const jobBoxes = document.querySelectorAll(".box1");

    seeJobsButtons.forEach(button => {
      button.addEventListener("click", function(event) {
        event.preventDefault();
        const clickedCategory = this.getAttribute("data-category");

        jobBoxes.forEach(box => {
          if (box.classList.contains(clickedCategory + "-jobs")) {
            box.style.display = "block"; // Set the appropriate display value
          } else {
